...
func TestClientRecharge(t *testing.T) {
	...
	t.Run("Existing Invoice Conf", func(t *testing.T) {
		db, mock, err := sqlmock.New()
		if err != nil {
			t.Fatalf("Error starting mock DB: %s", err)
		}
		...
		invoiceConfKey := fmt.Sprintf("count_%d", time.Now().Year())

		mock.ExpectQuery("SELECT \\* FROM cc_invoice_conf WHERE key_val = ?").
			WithArgs(invoiceConfKey).
			WillReturnRows(sqlmock.NewRows([]string{"id", "key_val", "value"}).
				AddRow(1, invoiceConfKey, "1"))

		mock.ExpectExec("UPDATE cc_invoice_conf").
			WithArgs("2", invoiceConfKey).
			WillReturnResult(sqlmock.NewResult(1, 1))

		description := "Invoice for refill (api)"
		reference := fmt.Sprintf("%d%08d", time.Now().Year(), 2)
		mock.ExpectExec("INSERT INTO cc_invoice").
			WithArgs(AnyTime{}, args["cardID"], "REFILL", reference, description, 1, 1).
			WillReturnResult(sqlmock.NewResult(1, 1))

		mock.ExpectExec("INSERT INTO cc_invoice_payment").
			WithArgs(1, 1).
			WillReturnResult(sqlmock.NewResult(1, 1))

		mock.ExpectExec("INSERT INTO cc_invoice_item").
			WithArgs(AnyTime{}, 1, float64(args["amount"].(int)), float64(0), description).
			WillReturnResult(sqlmock.NewResult(1, 1))
	...
	t.Run("New Invoice Conf", func(t *testing.T) {
		db, mock, err := sqlmock.New()
		if err != nil {
			t.Fatalf("Error starting mock DB: %s", err)
		}
		...
		mock.ExpectExec("INSERT INTO cc_invoice_conf").
			WithArgs(invoiceConfKey, "1").
			WillReturnResult(sqlmock.NewResult(1, 1))

		description := "Invoice for refill (api)"
		reference := fmt.Sprintf("%d%08d", time.Now().Year(), 1)
		mock.ExpectExec("INSERT INTO cc_invoice").
			WithArgs(AnyTime{}, args["cardID"], "REFILL", reference, description, 1, 1).
			WillReturnResult(sqlmock.NewResult(1, 1))
